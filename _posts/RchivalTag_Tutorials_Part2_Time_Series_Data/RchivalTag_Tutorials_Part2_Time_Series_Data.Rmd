---
title: "RchivalTag Tutorial | Part 2 (Interactive) Time Series Data Plots"
description: |
...
Fortunately, `RchivalTag` has it's own function to read time series data from different manufacturers (e.g. [Wildlife Computers](https://wildlifecomputers.com/) and [LOTEK](https://www.lotek.com/)) that fills gaps with `NAs` and does the datetime conversion automatically.
...
As mentioned earlier, `read_TS` also works with data from other manufacturers (currently only [LOTEK](https://www.lotek.com/)). Here is the same data in the [LOTEK](https://www.lotek.com/) PSAT Dive format:
...
This worked out, however, [LOTEK](https://www.lotek.com/) does not include the DeployID and Ptt information in their time series data. We therefore need to add this information manually. This will be important when further converting and visualizing our data.
...
`RchivalTag` fills gaps with NAs. However, both `plot_DepthTS` and `dy_DepthTS` function can also deal with data data gaps:
...
Another option is to estimate the temperature profiles from external data sets. PDT-data (PAT-Style Depth-Temperature profiles) from [Wildlife Computers](https://wildlifecomputers.com/) are one example for that.
