---
title: "RchivalTag Tutorial | Part 3 - Daily Depth-Temperature profiles"
description: |
...
setwd(path)
ts_raw <- read_TS("104659-Series.csv")
dates <- unique(ts_raw$date)
ts_sub <- ts_raw[which(ts_raw$date == dates[5]),]
bin_breaks <- seq(0,ceiling(max(ts_sub$Depth)/8)*8,by=8)
tad <- ts2histos(ts_sub,tad_breaks = bin_breaks)
hist_tad(tad,do_mid.ticks = F,main = "", plot_ntags = F, plot_nrec = F)

library(dplyr)
pdt <- bin_TempTS(ts_sub) %>% select(Depth, nrecs, MinTemp, MaxTemp)
pdt$reported <- F
pdt$reported[c(1,nrow(pdt))] <- T 
pdt$nrecs2 <- pdt$nrecs
pdt$nrecs2[c(1,nrow(pdt))] <- 0
pdt <- pdt[order(-pdt$nrecs2),]
pdt$reported[1:6] <- T
pdt <- pdt[order(pdt$Depth),]
pdt$nrecs2 <- c()
library(DT)
 pdt %>% datatable() %>%
    DT::formatStyle(columns= 1:ncol(pdt), fontSize = '70%')
```
...
But first, let's have a look on the format of a PDT file from WC: 
